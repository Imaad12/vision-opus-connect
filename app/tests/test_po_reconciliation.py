...
ever imported. `app.services.purchase_order_service.
reconcile_unmatched_purchase_orders` — invoked automatically by
...
from app.models import PurchaseOrder, Quotation
from app.services import client_service, project_service
from app.services.import_service import confirm_import, stage_document, stage_purchase_order_document
from app.services.purchase_order_service import reconcile_unmatched_purchase_orders
...
def test_po_before_quotation_is_automatically_linked_and_awarded(db_session: Session, tmp_path: Path) -> None:
    ...
    po_document = stage_purchase_order_document(db_session, po_path)
    assert po_document.purchase_order_candidate.match_status == PurchaseOrderMatchStatus.UNMATCHED
    ...
    candidate = po_document.purchase_order_candidate
    assert candidate.match_status == PurchaseOrderMatchStatus.MATCHED
...
def test_quotation_before_po_still_matches_at_extraction_time(db_session: Session, tmp_path: Path) -> None:
    ...
    assert po_document.purchase_order_candidate.match_status == PurchaseOrderMatchStatus.MATCHED
    assert po_document.purchase_order_candidate.matched_quotation_id == version.quotation_id
...
def test_reconciliation_with_nothing_to_reconcile_is_a_safe_no_op(db_session: Session) -> None:
    assert reconcile_unmatched_purchase_orders(db_session) == []
...
def test_reconciliation_to_an_ambiguous_reference_never_awards(db_session: Session, tmp_path: Path) -> None:
    ...
    db_session.refresh(po_document)
    candidate = po_document.purchase_order_candidate
    assert candidate.match_status == PurchaseOrderMatchStatus.AMBIGUOUS
...
def test_two_pos_staged_for_the_same_not_yet_existing_quotation_reconcile_idempotently(
    ...
    reconciled, but only one `PurchaseOrder` is ever created -- the
    ...
    po_document_a = stage_purchase_order_document(db_session, po_path_a)
    po_path_b = _write_po_txt(tmp_path, "po_b.txt", reference="VN/QU/996/25")
    po_document_b = stage_purchase_order_document(db_session, po_path_b, allow_duplicate=True)
    ...
    assert po_document_a.resulting_purchase_order_id == po_document_b.resulting_purchase_order_id
...
def test_reconciliation_never_reprocesses_an_already_confirmed_po(db_session: Session, tmp_path: Path) -> None:
    ...
    `reconcile_unmatched_purchase_orders` is called again afterward
    ...
    `PurchaseOrder` and no second award attempt are possible."""
    po_path = _write_po_txt(tmp_path, "po.txt", reference="VN/QU/995/25")
    stage_purchase_order_document(db_session, po_path)
    ...
    # PO at all.
    reconciled_again = reconcile_unmatched_purchase_orders(db_session)
